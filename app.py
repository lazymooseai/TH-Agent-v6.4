...
def get_trains(asema_nimi):
    ...
    try:
        ...
        return [{"train": "API-virhe", "origin": "Ei yhteyttä VR-rajapintaan.", "time": "", "delay": 0, "dt": datetime.datetime.now()}]
...
def tunnista_terminaali(teksti, laiva_nimi="", aika=""):
    ...
    # PDF:n KRIITTINEN SÄÄNTÖ: Eckerö Line MS Finlandia saapuu Länsisatamaan T2, ei Vuosaareen
            ...
            if isinstance(data, list) and data:
                ...
                for lento in data:
                    ...
                    tulos.append({
                        ...
                        "type": "OpenSky-data",
                        "wb": False,
                        "status": "Saapunut/Lähestyy"
                    })
                tulos.sort(key=lambda x: x["time"])
                return tulos[:8], "⚠️ Finavia API ei vastaa – näytetään rajattua avointa dataa."
    except:
        pass
...
# ==========================================
# TAPAHTUMAT (Yksinkertaistettu vikasietoiseksi)
# ==========================================
...
def yhdista_urheiludata(paikat, pvm_iso: str):
    ...
    for p in paikat:
        ...
        if "hifk" in nimi:
            ...
            # Mestistä ei ole Liigan APIssa, näytetään suora linkki pdf-ohjeen mukaan
...
def render_dashboard():
    ...
    with tab1:
        kulttuuri_paikat = [
            {"nimi": "Helsingin Kaupunginteatteri (HKT)", "kap": "947 hlö", "huomio": "Yleensä ti-su klo 19", "linkki": "https://hkt.fi/kalenteri/"},
            {"nimi": "Kansallisooppera ja baletti", "kap": "1 700 hlö", "huomio": "Yleensä ti-su klo 18/19", "linkki": "https://oopperabaletti.fi/ohjelmisto-ja-liput/"},
            {"nimi": "Kansallisteatteri", "kap": "1 000 hlö", "huomio": "Yleensä ti-su klo 19", "linkki": "https://kansallisteatteri.fi/esityskalenteri"},
            {"nimi": "Musiikkitalo", "kap": "1 704 hlö", "huomio": "Konsertit usein klo 19", "linkki": "https://musiikkitalo.fi/tapahtumat/"},
            {"nimi": "Helsingin Suomalainen Klubi", "kap": "300 hlö", "huomio": "Yritystilaisuuksia", "linkki": "https://tapahtumat.klubi.fi/tapahtumat/"},
        ]
        st.markdown(f"<div class='taksi-card'>{venue_html(kulttuuri_paikat)}</div>", unsafe_allow_html=True)
